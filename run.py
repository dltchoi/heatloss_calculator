...
def calculator_output():
    ...
    # calculate heat loss with defined function
        result = hl_estimate(gasuse_total, dhw_monthly, balance_point, furnace_eff, derate, location, heatpumpsize, heatpumpcap_17_c1, heatpumpcap_47_c1, heatpumpcap_17_c2, heatpumpcap_47_c2, heatpumpcap_17_c3, heatpumpcap_47_c3, hp_derate)
        ...
        if m_hp_1 > 0:
            ...
            p_hp = round((heatload_dist.loc[heatload_dist.temp >= switchtemp_1].GJ.sum()/GJ_total)*100)
            p_f = 100 - p_hp
            ...
            curve1_result = zip([switchtemp_1],[p_hp],[p_ehp])
        else:
            switchtemp_1 = 'None'
            curve1 = []
            curve1_result = []
        ...
        if m_hp_2 > 0:
            ...
            p_hp = round((heatload_dist.loc[heatload_dist.temp >= switchtemp_2].GJ.sum()/GJ_total)*100)
            p_f = 100 - p_hp
            # calculate percentage of heating provided by heat pump and elec resistance
            p_e = round((heatload_dist.e_req_belowswitch.sum()/kbtu_total)*100)
            p_ehp = round((heatload_dist.e_hp_belowswitch.sum()/kbtu_total)*100) + p_hp
            ...
            curve2_result = zip([switchtemp_2],[p_hp],[p_ehp])
        else:
            switchtemp_2 = []
            curve2 = []
            curve2_result = []
        ...
        if m_hp_3 > 0:
            ...
            p_hp = round((heatload_dist.loc[heatload_dist.temp >= switchtemp_3].kbtu.sum()/kbtu_total)*100)
            p_f = 100 - p_hp
            # calculate percentage of heating provided by heat pump and elec resistance
            p_e = round((heatload_dist.e_req_belowswitch.sum()/kbtu_total)*100)
            p_ehp = round((heatload_dist.e_hp_belowswitch.sum()/kbtu_total)*100) + p_hp
            ...
            curve3_result = zip([switchtemp_3],[p_hp],[p_ehp])
        else:
            switchtemp_3 = []
            curve3 = []
            curve3_result = []
        ...
        ax4.grid()
        ax4.set_title('Heating Load Distribution for 2021', fontsize=19)
    ...
    return render_template('calculator_output.html', heatloss_url=heatloss_url, heatdist_url=heatdist_url, dtemp=dtemp, heatloss_design=round(heatloss_design), gasuse_total=int(gasuse_total), dhw_annual=int(dhw_monthly*12), balance_point=int(balance_point), furnace_eff=furnace_eff, derate=derate, hp_derate=hp_derate, location=location.capitalize(), heatpumpsize=heatpumpsize, switchlist=switchtemp_list, curve1=curve1, curve2=curve2, curve3=curve3, curve1_result=curve1_result, curve2_result=curve2_result, curve3_result=curve3_result, ziplist=ziplist)
